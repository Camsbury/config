...
let
  ...
  # The greeter's password box is hardcoded to screen center (mini-greeter
  # src/ui.c place_main_window; no offset config exists), so to keep the logo
  # VISIBLE we move the logo instead of the box. Shift the base image up
  ...
  # The one shared wallpaper. Three transforms, in order:
  ...
  #   2. shift the logo up by `logoShift` px: crop the top off and pad the same
  #      amount of black at the bottom, lifting the dead-center logo above the
  #      centered password box. Must come AFTER the blacken so the padded strip
  #      is pure black, and BEFORE the resize so the shift is in base px.
  #   3. cover-fit to the panel: the greeter's centered-unscaled draw needs a
  #      panel-sized image to fill edge to edge.
  wallpaper = pkgs.runCommand "desktop-wallpaper.png" { } ''
    ${pkgs.imagemagick}/bin/magick ${baseWallpaper} \
      -black-threshold 40% \
      -gravity North -background black \
      -crop 1920x${toString (1080 - logoShift)}+0+${toString logoShift} +repage \
      -extent 1920x1080 \
...
in
{
  options.ck.theme = {
    ...
    wallpaper = lib.mkOption {
      ...
      readOnly = true;
      description = ''
        The shared processed wallpaper: pure-black background (logo kept),
        cover-scaled to the panel. Used by the lock screen and login greeter.
      '';
    };
  };
